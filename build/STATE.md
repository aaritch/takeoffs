...
- **Active task:** **P1-01..P1-11 done** (upload → ingest pipeline → processing UI → deep-zoom viewer + overlay → measurement tools **wired end-to-end via the toolbar** → rollups). **9/14 Phase 1 DONE.** P1-06 viewer is built+tested+smoked but its GATE stays open on human perf-on-hardware.
- **Next up:** **P1-12 — Undo/redo**, then reports P1-13 + export-parity gate P1-14. Also pending: a real upload-client UI (P1-01 has the API), the standalone `apps/worker-files` deployable (§6), and the human viewer perf/browser gate check.
...
### Phase 1 — Manual Takeoff  (progress: 9/14 DONE)
