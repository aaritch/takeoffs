...
- **Last updated:** 2026-06-19, aarit — **P1-06 (tiled viewer · GATE) — engine + serving + canvas built & tested; perf-on-hardware verification pending.** Pure **viewer engine** (`viewport.ts` immutable world↔screen transform — shared with overlay P1-07; `dzi.ts` level selection + visible-tile math matching the producer's DZI) — **11 tests**. **Tile-serving**: `GET /v1/sheets/{id}/tiles/{...path}` (authorized, RLS-scoped, path-traversal-safe — `getTileObject` service, **3 tests incl. org isolation**) + `GET /v1/sheets/{id}`. **`TileViewer`** canvas component: pan (pointer) + zoom (cursor-anchored wheel), overview-first paint, per-level visible-tile loading with an image cache, devicePixelRatio-aware (jsdom mount test). **Full suite 106/106 green.** **GATE still open** — the perf budget (60fps pan/zoom, <300ms first paint) must be checked on representative hardware (can't measure headlessly), and a viewer page + live visual smoke against a processed sheet are still to wire. Phase 1 **5/14 DONE** (P1-06 IN_PROGRESS).
...
- [~] P1-06 — Viewer engine (viewport transform + DZI tile selection), authorized tile-serving, and the pan/zoom canvas component are **built & tested** (12 viewer tests + 3 tile-serving). **Performance budget (60fps pan/zoom, <300ms first paint) NOT yet verified on representative hardware** — the gate's real pass condition, and a human/manual step (no headless fps measurement). A viewer page + live visual smoke against a processed sheet also remain.
