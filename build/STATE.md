...
- **Active task:** **Phase 1 functionally COMPLETE — P1-01..P1-14 all DONE** (upload → ingest pipeline → processing UI → deep-zoom viewer + overlay → measurement tools via the toolbar → rollups → undo/redo → reports/exports → **export-parity gate**). **14/14 Phase 1 DONE.** Only remaining Phase-1 caveat: the **P1-06 viewer perf GATE** stays open pending a human perf-on-hardware check (can't be measured headlessly).
- **Next up:** **Phase 2 — AI takeoff with human review** (P2-01 stage contracts first). NOTE: Phase 2 needs the **second compute home** (GPU + standalone workers) chosen — that's the gating decision before P2-02 (GPU pool) and the extraction of `apps/worker-files`/`worker-exports` as deployables. Also still pending from Phase 1: a real upload-client UI (P1-01 has the API), MARKED_PLANS raster export, and the human viewer perf/browser gate check.
