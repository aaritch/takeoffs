...
- **Active task:** **P1-01..P1-13 done** (upload → ingest pipeline → processing UI → deep-zoom viewer + overlay → measurement tools via the toolbar → rollups → undo/redo → **reports/exports**). **11/14 Phase 1 DONE.** P1-06 viewer is built+tested+smoked but its GATE stays open on human perf-on-hardware.
- **Next up:** **P1-14 — Export-vs-rollup parity (GATE)** — an automated check that every number in every export reconciles to the authoritative rollup pre-rounding; the P1-13 renderer already pulls straight from the rollup, so this is mostly a dedicated parity test + a deliberately-introduced-discrepancy negative test. Then Phase 1 is complete. Also pending: a real upload-client UI (P1-01 has the API), the standalone `apps/worker-files`/`worker-exports` deployables (§6), MARKED_PLANS raster export, and the human viewer perf/browser gate check.
...
### Phase 1 — Manual Takeoff  (progress: 11/14 DONE)
