...
- **Last updated:** 2026-06-19, aarit — **P1-07 (vector overlay & selection) DONE.** Overlay renders measurements in a **canvas layer sharing the SAME `Viewport`** as the tiles (so geometry stays pixel-aligned at every zoom — never stores screen coords). **Hit-testing against vector geometry** (`hit-test.ts`: point/polyline/polygon-with-holes distance; `pickAtScreen` uses a zoom-scaled, constant-screen-pixel tolerance → "picks the right one near others at any zoom") — **6 tests**. New **`LayeredViewer`** composes tile + overlay on one viewport with pan/zoom, drag-vs-click discrimination, color-by-condition, and (multi-)select; `drawVectors` renderer; tile draw extracted to `tile-render.ts` (shared). Consolidated the standalone `TileViewer` into `LayeredViewer`; `SheetViewer` now uses it (measurements populate when P1-09 lands). **Full suite 112/112 green** (18 viewer tests). Phase 1 now **6/14**. (Shares P1-06's human perf-on-hardware gate: "thousands of objects without stalling" + 60fps.)
...
### Phase 1 — Manual Takeoff  (progress: 6/14 DONE)
