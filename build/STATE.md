...
- **Active task:** **P1-01..P1-07 done** (upload → ingest pipeline → processing UI → deep-zoom viewer + vector overlay). **6/14 Phase 1 DONE.** P1-06 viewer is built+tested+smoked but its GATE stays open on human perf-on-hardware; P1-08 geometry pkg is done (calibration UI pending the viewer).
- **Next up:** **P1-09 — Manual measurement tools** (draw point/line/area on the overlay → create measurements; pairs with the P1-08 scale-calibration UI). Then undo/redo P1-12, reports P1-13. Also pending: a real upload-client UI (P1-01 has the API), the standalone `apps/worker-files` deployable (§6), and the human viewer perf/browser gate check.
...
### Phase 1 — Manual Takeoff  (progress: 7/14 DONE)
