...
- **Active task:** **P1-01..P1-12 done** (upload → ingest pipeline → processing UI → deep-zoom viewer + overlay → measurement tools via the toolbar → rollups → **undo/redo**). **10/14 Phase 1 DONE.** P1-06 viewer is built+tested+smoked but its GATE stays open on human perf-on-hardware.
- **Next up:** **P1-13 — Reports & exports** (background jobs in `worker-exports`; needs the standalone worker host), then export-parity gate P1-14. Also pending: a real upload-client UI (P1-01 has the API), the standalone `apps/worker-files`/`worker-exports` deployables (§6), and the human viewer perf/browser gate check.
...
### Phase 1 — Manual Takeoff  (progress: 10/14 DONE)
