...
- **Active task:** Phase 1 ingestion is built end-to-end (**P1-01..P1-05 done**: upload → scan → split → rasterize+tile → extract metadata → processing status/retry UI). **5/14 Phase 1 DONE.**
- **Next up:** **P1-06 — Tiled viewer canvas (GATE)** — the deep-zoom viewer that renders the DZI tiles + thumbnails P1-03 produces (conventions in `@takeoff/contracts/tiles`). Then overlay P1-07, scale-calibration UI (P1-08 pkg done), tools P1-09. Also pending: a real upload-client UI (P1-01 has the API), and the standalone `apps/worker-files` deployable (§6).
...
### Phase 1 — Manual Takeoff  (progress: 5/14 DONE)
