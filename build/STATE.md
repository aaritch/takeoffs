...
- **Last updated:** 2026-06-17, aarit — **Cloudflare R2 live & wired.** `storage:check` passes against the real `takeoffs` bucket (put→signed-download→signed-upload→delete); CORS applied via `storage:setup-cors` (`PutBucketCors`, currently `localhost:3000`). Repo linked to Vercel project `aaritchs-projects/takeoffs`; the five `S3_*` vars set in Vercel across **Production/Preview/Development** (Preview added via REST API — CLI's agent-non-interactive mode blocks the "all Preview branches" default). Health route reports `storage` (was stale `blob`). Remaining for prod: add prod domain to R2 CORS; mirror the *other* integration vars (Neon/Upstash/OIDC) into Vercel when those accounts exist.
...
| 2026-06-15 | P0-04 (hosted), P0-05, P0-08 | Hosted provisioning + OIDC login need cloud accounts | ~~GitHub repo~~, ~~app shell~~, ~~integration code wiring~~ DONE (Auth.js OIDC, Redis, S3 storage all wired + tested where possible). **Remaining is account creation + env only:** an OIDC provider, Neon (+ non-superuser app role), Upstash, an S3 bucket (R2/S3) — step-by-step in `docs/runbooks/integrations-setup.md`. Then set the env in Vercel and deploy. | aarit | OPEN (only account/env setup left) |
...
- [~] Dev environment (P0-04): **local** docker-compose stack up & PostGIS spatial column verified (create/insert/drop, reset+up repeatable); **hosted** object storage = **Cloudflare R2, live** (round-trip verified; `S3_*` set in Vercel Prod/Preview/Dev; repo linked to `aaritchs-projects/takeoffs`). Neon/Upstash + OIDC provisioning still pending cloud creds.
